...
const ManageRequests = () => {
  ...
  const handleAction = async (email, action) => {
    try {
      const res = await axiosSecure.patch(`/role-request/${email}`, { action });
      ...
      if (err.response?.status === 409) {
        Swal.fire("Info", "Request already processed", "info");
      } else {
  ...
  const isDisabled = (req) => req.requestStatus !== "pending";
